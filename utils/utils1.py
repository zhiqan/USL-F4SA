...
from dataset.data_CWRU import CWRUDataset1
from dataset.data_PU import PUDataset
from dataset.data_HIT import HITDataset
...
def build_fewshot_loader(args: dict, mode: str = 'test', max_n_shot: int = 5):
    ...
    test_dataset= CWRUDataset1(root, mode=mode)  

    _, test_dataset.targets = np.unique(
            test_dataset.targets, return_inverse=True)


    test_sampler = EpisodeSampler(
        test_dataset.targets, args.n_test_task//args.test_batch_size, args.n_way,
...
def build_fewshot_loader_PU(args: dict, mode: str = 'test', max_n_shot: int = 5):
    """ 
    Generates an episodic FSL dataloader for validation & testing.
    ...
    """
    assert mode in ['train', 'val']
    #combined_array= np.load('D:\\自监督长尾故障诊断\\PUSHUJU\\train_1024.npy')
    Test_Data = np.load('D:\\无监督元学习_数据裁剪_域外数据\BECLR-main\\PUdata\\test_1024.npy')
    
    #Test_Data = np.load('D:\\自监督长尾故障诊断\\PUSHUJU\\test_FFT1024.npy')
    #combined_array= np.load('D:\\航空发动机数据\\HIT_hangkong_train.npy')
    #combined_array = np.concatenate((Train_Data, Test_Data), axis=0)
    aa=Test_Data.shape[1]

    Test_Data_array=Test_Data.reshape(Test_Data.shape[0]*Test_Data.shape[1],Test_Data.shape[3])
    #Test_Data_array=Test_Data.reshape(Test_Data.shape[0]*Test_Data.shape[1],Test_Data.shape[2])


    num_samples = Test_Data_array.shape[0]
    num_classes = 19
    samples_per_class = aa
    ...
    test_dataset=PUDataset(Test_Data_array, labels_array, mode='val',index='index', cls_num=28,imb_type='exp', imb_factor=1) 
...
def build_fewshot_loader_HIT(args: dict, mode: str = 'test', max_n_shot: int = 5):
    """ 
    Generates an episodic FSL dataloader for validation & testing.
    ...
    Test_Data_array=Test_Data.reshape(Test_Data.shape[0]*Test_Data.shape[1],Test_Data.shape[2],Test_Data.shape[3])
    ...
    # 确保标签数量与 num_samples 匹配
    labels_array = np.tile(labels_array, num_samples // total_labels) 
    test_dataset=HITDataset(Test_Data_array, labels_array, mode='val',index='index', cls_num=19,imb_type='exp', imb_factor=0.1) 
...
def load_student_teacher(student: nn.Module,
    ...
    del checkpoint
    return student, teacher, optimizer, epoch, loss, batch_size
